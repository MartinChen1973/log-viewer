...
def _mcp_connections_from_env() -> dict[str, dict[str, Any]]:
    ...
    if os.environ.get("MCP_ENABLED", "true").strip().lower() in ("0", "false", "no", "off"):
        return {}
    ## Downstream MCP defaults: OA 28081, Bingchuan 28083, aux 28082, log-analyzer 28084 (86xx–87xx often excluded on Windows Hyper-V/WSL).
    u_oa = (os.environ.get("MCP_OA_URL") or "http://127.0.0.1:28081/mcp").strip()
    u_bc = (os.environ.get("MCP_BINGCHUAN_URL") or "http://127.0.0.1:28083/mcp").strip()
    u_aux = (os.environ.get("MCP_AUX_URL") or "http://127.0.0.1:28082/mcp").strip()
    u_log = (os.environ.get("MCP_LOG_ANALYZER_URL") or "http://127.0.0.1:28084/mcp").strip()
    if os.environ.get("MCP_LOG_ANALYZER_ENABLED", "true").strip().lower() in ("0", "false", "no", "off"):
        u_log = ""
    ...
    if u_log:
        out["log_analyzer"] = {"transport": "http", "url": u_log}
    return out
...
def _prompt_for_log_analysis(
    ...
    *,
    ...
    return (
        ...
        "Write the entire answer in Markdown using **Simplified Chinese** for explanations, "
        "headings, and narrative. Keep **verbatim English** for critical technical tokens taken "
        "from the logs: error/warning keywords, HTTP methods and paths, status codes, hostnames, "
        "IPs, file paths, class or logger names, SQL fragments, stack frames, and any quoted "
        "substrings from the log lines. Do not translate those tokens; you may gloss them in Chinese "
        "after the English term in parentheses if helpful.\n\n"
        "Use this structure (section titles may be Chinese only or Chinese with English in parentheses):\n"
        "1. **摘要 (Summary)** — 本段日志在做什么、整体健康度。\n"
        "2. **问题与异常 (Issues)** — 错误、告警、异常；引用日志中的原文短片段（英文保留原样）。\n"
        "3. **可能原因 (Likely causes)** — 简要推测。\n"
        "4. **建议下一步 (Next steps)** — 可执行的排查或修复步骤。\n\n"
        "--- log excerpt ---\n"
        f"{body.content}\n"
        "--- end ---"
    )
